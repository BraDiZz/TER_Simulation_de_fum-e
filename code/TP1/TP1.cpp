...
void particles_clear(Particle &p){
    p.position.clear();
    p.life.clear();
    p.deplacement.clear();
...
void particles_erase(Particle &p,int i){
    p.position.erase(p.position.begin()+i);
    p.deplacement.erase(p.deplacement.begin()+i);
    p.life.erase(p.life.begin()+i);
    p.baseVelocity.erase(p.baseVelocity.begin()+i);
}
...
int main( void )
{
    ...
    do{
        ...
        glUniform1i(glGetUniformLocation(programID,"currentMesh"),currentMesh);
        glUniform3f(glGetUniformLocation(programmeID,"wind"),windVector[0],windVector[1],windVector[2]);
        glUniform1f(glGetUniformLocation(programmeID,"gravity"),gravityVector.y);
        ...
        if(currentMesh==1){
            ...
            if(generate && !sphere_generate){
                ...
                /*
                particles.position.resize(indexed_vertices_mesh.size());
        ...
        glGenBuffers(2, &deplacementBuffer);
        glBindBuffer(GL_ARRAY_BUFFER, deplacementBuffer);
        glBufferData(GL_ARRAY_BUFFER, particles.position.size() * sizeof(glm::vec3), nullptr, GL_DYNAMIC_DRAW);
