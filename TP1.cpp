...
float calculateLocalDensity(const std::vector<glm::vec3>& particlePositions, int particleIndex, float cellSize) {
    glm::vec3 position = particlePositions[particleIndex];
    float count = 0.0f;
    for (const auto& otherPosition : particlePositions) {
        if (glm::distance(position, otherPosition) < cellSize) {
            count += 1.0f;
        }
    }
    float volume = (4.0f / 3.0f) * glm::pi<float>() * glm::pow(cellSize, 3);
    return count / volume;
}

float calculateAverageDensity(const std::vector<glm::vec3>& particlePositions, float cellSize, int resolution) {
    float totalVolume = glm::pow(cellSize * resolution, 3);
    return static_cast<float>(particlePositions.size()) / totalVolume;
}
float diffusionRate = 0.5f;

void advectParticles(const std::vector<std::vector<std::vector<glm::vec3>>>& velocityField,std::vector<glm::vec3>& particlePositions,float dt,float cellSize,int resolution, float diffusionRate) {
    for(int m=0;m<particlePositions.size();m++){
        int i = static_cast<int>((particlePositions[m][0] >= 0 ? particlePositions[m][0] : -particlePositions[m][0]) / cellSize); // Utilisation de la valeur absolue pour obtenir l'indice
        int j = static_cast<int>((particlePositions[m][1] >= 0 ? particlePositions[m][1] : -particlePositions[m][1]) / cellSize);
        int k = static_cast<int>((particlePositions[m][2] >= 0 ? particlePositions[m][2] : -particlePositions[m][2]) / cellSize);
        if(i >= 0 && i < velocityField.size() && j >= 0 && j < velocityField[i].size() && k >= 0 && k < velocityField[i][j].size()) {
            ...
            // Ajout du terme de diffusion
            float localDensity = calculateLocalDensity(particlePositions, m, cellSize);
            float averageDensity = calculateAverageDensity(particlePositions, cellSize, resolution);
            float diffusionValue = diffusionRate * (averageDensity - localDensity) * dt;
            glm::vec3 diffusion = glm::vec3(diffusionValue, diffusionValue, diffusionValue);
            particlePositions[m] += diffusion;
        }
    }
}
...
// // }
void correctVelocityWithPressure(std::vector<std::vector<std::vector<glm::vec3>>>& velocityField,const std::vector<std::vector<std::vector<float>>>& pressureField,float cellSize, float viscosity, glm::vec3 gravity) {
    int resolution = pressureField.size();
    ...
    for (int i = 1; i < resolution - 1; ++i) {
        for (int j = 1; j < resolution - 1; ++j) {
            for (int k = 1; k < resolution - 1; ++k) {
                glm::vec3 pressureGradient = glm::vec3(
                    (pressureField[i+1][j][k] - pressureField[i-1][j][k]) / (2 * cellSize),
                    (pressureField[i][j+1][k] - pressureField[i][j-1][k]) / (2 * cellSize),
                    (pressureField[i][j][k+1] - pressureField[i][j][k-1]) / (2 * cellSize)
                );

                glm::vec3 viscosityTerm = glm::vec3(
                    viscosity * (velocityField[i+1][j][k].x - 2*velocityField[i][j][k].x + velocityField[i-1][j][k].x) / (cellSize*cellSize),
                    viscosity * (velocityField[i][j+1][k].y - 2*velocityField[i][j][k].y + velocityField[i][j-1][k].y) / (cellSize*cellSize),
                    viscosity * (velocityField[i][j][k+1].z - 2*velocityField[i][j][k].z + velocityField[i][j][k-1].z) / (cellSize*cellSize)
                );

                velocityField[i][j][k] -= pressureGradient + viscosityTerm + gravity;
            }
        }
    }
}
...
/*
    ...
    float viscosity = 0.1f; // Définit une viscosité relativement faible
    ...
    do{
        ...
        correctVelocityWithPressure(velocityField,pressureField, side/resolution, viscosity, gravityVector);
        advectParticles(velocityField,particles.position,scalarForce,side/resolution,resolution, diffusionRate);
